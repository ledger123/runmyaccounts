...
%source = (
	    1 => { url => "http://www.runmyaccounts.com/source", site => "www.runmyaccounts.com", locale => us },
            2 => { url => "http://abacus.runmyaccounts.com/source", site => "abacus.runmyaccounts.com", locale => ca },
	  );
...
if ($filename) {
  # extract version
  die "Not a Run my Accounts archive\n" if ($filename !~ /^sql-ledger/);
...
$install .= "\n(d)ownload $latest_version (no installation)" unless $filename;
  ...
  print qq|
               ...
               Run my Accounts ERP Installation
