    ...
    if ( @{ $form->{all_department} } ) {
        if ( $myconfig{department_id} and $myconfig{role} eq 'user' ) {
            ...
            for ( @{ $form->{all_department} } ) {
                $form->{selectdepartment} .= qq|<option value="| . $form->quote( $_->{description} ) . qq|--$_->{id}">$_->{description}\n|;
            }
        }
    }
...
| if $form->{selectdepartment};
    ...
    if ( @{ $form->{all_years} } ) {
        ...
        for ( sort keys %{ $form->{all_month} } ) {
            $selectaccountingmonth .= qq|<option value=$_>| . $locale->text( $form->{all_month}{$_} ) . qq|\n|;
        }
...
|;
    ...
    if ( @{ $form->{all_project} } ) {
        ...
        for ( @{ $form->{all_project} } ) {
            $form->{selectproject} .= qq|<option value="| . $form->quote( $_->{projectnumber} ) . qq|--$_->{id}">$_->{projectnumber}\n|;
        }
	...
	<tr>
	  ...
	  <td colspan=3><select name=language_code>|
          . $form->select_option( $form->{selectlanguage}, $form->{language_code}, undef, 1 ) . qq|</select></td>
	</tr>|;
        ...
        || $form->{report} eq 'income_statement' )
    {
	    ...
	    <td><select name=currency>|
              . $form->select_option( $form->{selectcurrency}, $form->{defaultcurrency} ) . qq|</select></td>
	  </tr>|
...
|;
    ...
    if ( $form->{report} eq "projects" ) {
	  ...
	  <td colspan=3><input name=fromdate size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)" value=$form->{fromdate}> <b>|
          . $locale->text('To') . qq|</b> <input name=todate size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)"></td>
	</tr>
	$selectfrom
      </table>
    </td>
...
|;
	...
	<tr>
	  ...
	  <td colspan=3><input name=comparefromdate size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)"> <b>|
          . $locale->text('To') . qq|</b> <input name=comparetodate size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)"></td>
	</tr>
|;
        ...
        if ( @{ $form->{"all_$form->{vc}"} } ) {
	     ...
	     <td colspan=2><select name=$form->{vc}><option>\n|;
            ...
            for ( @{ $form->{"all_$form->{vc}"} } ) {
                $vc .= qq|<option value="| . $form->quote( $_->{name} ) . qq|--$_->{id}">$_->{name}\n|;
            }
...
|;
        ...
        push @a, qq|<input name="l_variance" class=checkbox type=checkbox value=Y> | . $locale->text('Variance');
...
sub generate_balance_sheet {
    ...
    $form->{asofdate} = $form->current_date( \%myconfig )
      ...
      or ( $form->{asofmonth} and $form->{asofyear} );
    $form->{period} =
      $locale->date( \%myconfig, $form->current_date( \%myconfig ), 1 );
...
sub generate_trial_balance {
    ...
    for (qw(login path nextsub fromdate todate month year interval l_heading l_subtotal all_accounts l_name accounttype fx_transaction)) {
...
sub list_accounts {
    ...
    $column_header{debit}       = qq|<th align=right class=listheading width=10%>| . $locale->text('Debit') . qq|</th>|;
    $column_header{credit}      = qq|<th align=right class=listheading width=10%>| . $locale->text('Credit') . qq|</th>|;
    $column_header{begbalance}  = qq|<th align=right class=listheading width=10%>| . $locale->text('Beginning Balance') . qq|</th>|;
    $column_header{endbalance}  = qq|<th align=right class=listheading width=10%>| . $locale->text('Ending Balance') . qq|</th>|;
    ...
    if ( $form->{l_csv} ) {
        ...
        foreach $ref ( sort { $a->{accno} cmp $b->{accno} } @{ $form->{TB} } ) {
            ...
            $line = '';
            for (@column_index) {
                if ( $_ =~ /(begbalance|debit|credit|endbalance)/ ) {
                    $line .= qq|$ref->{$_},|;
                } else {
                    $line .= qq|"$ref->{$_}",|;
                }
...
qq|<th class=listheading width=1%><input name="allbox" type=checkbox class=checkbox value="1" $form->{allbox} onChange="CheckAll(); javascript:document.forms[0].submit()"><input type=hidden name=action value="$action"></th>|;
    ...
    $column_header{c0}                  = qq|<th align=right class=listheading width=10% nowrap>| . $locale->text('Current') . qq|</th>|;
    ...
    $column_header{total}               = qq|<th align=right class=listheading width=10% nowrap>| . $locale->text('Total') . qq|</th>|;
        ...
        if ( $vc_id != $ref->{vc_id} ) {
            ...
            if ( $form->{selectlanguage} ) {
                ...
                $column_data{language} =
                  qq|<td><select name="language_code_$ref->{curr}_$ref->{vc_id}">| . $form->select_option( $form->{selectlanguage}, $ref->{language_code}, undef, 1 ) . qq|</select></td>|;
            }
    ...
    while ( @{ $form->{AG} } ) {
        ...
        if ( $vc_id != $ref->{vc_id} ) {
            ...
            if ( $form->{"ndx_$ref->{curr}_$ref->{vc_id}"} ) {
                ...
                for (qw(invnumber ordnumber ponumber notes invdate duedate invdescription)) {
                    $form->{$_} = ();
                }
                $form->{total} = 0;
                ...
                for ( "c0", "c15", "c30", "c45", "c60", "c75", "c90", "" ) {
                    $form->{"${_}total"} = $form->format_amount( \%myconfig, $form->{"${_}total"}, $form->{precision} );
                }
...
sub list_payments {
    ...
    $column_header{variance}            = "<th><a class=listheading href=$href&sort=variance>" . $locale->text('Variance') . "</a></th>";
