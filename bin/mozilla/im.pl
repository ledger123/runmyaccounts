...
sub import {
	...
	|;
	  ...
	  <td>
	    <select name=paymentaccount>| . $form->select_option($selectpaymentaccount) . qq|</select>
	  </td>
	</tr>
	<tr>
	  ...
	  <td>
	    <select name=arapaccount>| . $form->select_option($selectarapaccounts) . qq|</select>
	  </td>
	</tr>| if $form->{ARAP};
	  ...
	  <td>
	    <select name=incomeaccount>| . $form->select_option($selectincomeaccounts) . qq|</select>
	  </td>
	  ...
	  <td>
	    <select name=expenseaccount>| . $form->select_option($selectexpenseaccounts) . qq|</select>
	  </td>
	  ...
	  <td>
	    <select name=IC_inventory>| . $form->select_option( $form->{"selectIC"} ) . qq|</select>
	  </td>
	</tr>
         <tr>
	  <th align=right>| . $locale->text('Income') . qq|</th>
	  <td>
	    <select name=IC_income>| . $form->select_option( $form->{"selectIC_income"} ) . qq|</select>
	  </td>
	</tr>
         <tr>
	  <th align=right>| . $locale->text('COGS') . qq|</th>
	  <td>
	    <select name=IC_expense>| . $form->select_option( $form->{"selectIC_expense"} ) . qq|</select>
	  </td>
	</tr>|;
	...
	<tr>
	   ...
	   <td>$tax</td>
        ...
        if ( $form->{selectcurrency} ) {
          ...
          <td><select name=curr>|
              . $form->select_option( $form->{selectcurrency}, $form->{curr} ) . qq|</select></td>\n|;
        }
        ...
        if ( $form->{selectarap} ) {
	        ...
	        <tr>
              ...
              . $form->select_option( $form->{selectarap}, $form->{arap_accno} ) . qq|</select>
	          </td>
	        </tr>
		|;
		...
		<tr>
	  	...
	  	<td><select name="payment_accno">|
              . $form->select_option( $form->{selectpayment}, $form->{payment_accno} ) . qq|</select>
	  	</td>
		</tr>
	    |;
        }
...
sub export {
    ...
    if ( $form->{type} eq 'payment' ) {
	  ...
	  <th align=right>| . $locale->text('Account') . qq|</th>
	  <td>
	    <select name=paymentaccount>| . $form->select_option( $form->{selectpaymentaccount} ) . qq|</select>
	  </td>
	</tr>
	<tr>
	  <th align=right nowrap>| . $locale->text('Currency') . qq|</th>
...
|;
	...
	<tr>
	  ...
	  <td><select name=paymentmethod>|
                  . $form->select_option( $form->{selectpaymentmethod}, $form->{paymentmethod}, 1 ) . qq|</select></td>
	</tr>
|;
            }
        }
...
sub im_sales_invoice {
    ...
    shift @flds;
    push @flds,
      qw(ordnumber quonumber customer_id datepaid dcn shippingpoint shipvia waybill terms notes intnotes language_code ponumber cashdiscount discountterms employee_id parts_id description sellprice discount qty unit serialnumber projectnumber deliverydate AR taxincluded department_id warehouse_id);
...
sub im_sales_order {
    ...
    shift @flds;
    push @flds,
      qw(ordnumber quonumber customer_id datepaid shippingpoint shipvia waybill terms notes intnotes language_code ponumber discountterms employee_id parts_id description sellprice discount qty unit serialnumber projectnumber deliverydate);
    unshift @column_index, "runningnumber";
...
sub im_purchase_order {
    ...
    shift @flds;
    push @flds,
      qw(ordnumber quonumber vendor_id datepaid shippingpoint shipvia waybill terms notes intnotes language_code ponumber discountterms employee_id parts_id description sellprice discount qty unit serialnumber projectnumber deliverydate);
    unshift @column_index, "runningnumber";
...
sub import_sales_invoices {
    ...
    for $k ( keys %ndx ) {
        ...
        if ( $form->{"ndx_$k"} ) {
            ...
            for (
                qw(invnumber ordnumber quonumber transdate customer customer_id datepaid duedate dcn shippingpoint shipvia waybill terms notes intnotes curr exchangerate language_code ponumber cashdiscount discountterms AR taxincluded)
              )
            {
                $newform->{$_} = $form->{"${_}_$k"};
            }
...
sub import_sales_orders {
    ...
    for $k ( keys %ndx ) {
        ...
        if ( $form->{"ndx_$k"} ) {
            ...
            for (
                qw(ordnumber quonumber transdate customer customer_id reqdate shippingpoint shipvia waybill terms notes intnotes curr language_code ponumber cashdiscount discountterms AR taxincluded))
            {
                $newform->{$_} = $form->{"${_}_$k"};
            }
...
sub import_purchase_orders {
    ...
    for $k ( keys %ndx ) {
        ...
        if ( $form->{"ndx_$k"} ) {
            ...
            for (qw(ordnumber quonumber transdate vendor vendor_id reqdate shippingpoint shipvia waybill terms notes intnotes curr language_code ponumber cashdiscount discountterms AP taxincluded)) {
                $newform->{$_} = $form->{"${_}_$k"};
            }
            $newform->{description} = $form->{"orderdescription_$k"};
...
sub im_partsvendor {
    ...
    print qq|<input name=action class=submit type=submit value="| . $locale->text('Import Parts Vendors') . qq|">
	</form></body></html>
  |;
...
sub im_vc {
    ...
    @flds = @column_index;
    push @flds,
      qw(cc bcc business_id taxnumber sic_code discount creditlimit employee_id language_code pricegroup_id curr cashdiscount threshold paymentmethod_id remittancevoucher contactid typeofcontact saluation occupation terms startdate mobile gender addressid shiptoname shiptoaddress1 shiptoaddress2 shiptocity shiptostate shiptozipcode shiptocountry shiptophone shiptofax shiptoemail bankname iban bic bankaddress1 bankaddress2 bankcity bankstate bankzipcode bankcountry dcn rvc membernumber);
    unshift @column_index, "runningnumber";
    ...
    while (1) {
        ...
        if ( $debitrow->{amount} and $creditrow->{amount} ) {
            if ( $debitrow->{amount} > $creditrow->{amount} ) {
                $form->{dbs}->query(
                    'insert into debitscredits (reference, department_id, description, transdate, debit_accno, credit_accno, amount) values (?, ?, ?, ?, ?, ?, ?)',
                    $debitrow->{reference},
                    $debitrow->{department_id},
...
|;
    ...
    if ( $form->{runit} ) {
        ...
        $table1 = $form->{dbs}->query($query)->xto(
