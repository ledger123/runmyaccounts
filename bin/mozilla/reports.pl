...
sub onhandvalue_list {
    ...
    while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
        $form->{link} =
          qq|$form->{script}?action=onhandvalue_detail&id=$ref->{id}&l_components=$form->{l_components}&l_sql=$form->{l_sql}&path=$form->{path}&login=$form->{login}&callback=$form->{callback}|;
...
sub gl_list {
    ...
    if ( $form->{l_csv} eq 'Y' ) {
        ...
        print
qq|<button onclick="window.parent.postMessage({name: 'ledgerEvent', params: {event: 'urlToPdf', url: window.location.href}}, '*')" class="noprint nkp" style="background-color: white; cursor: pointer; position: fixed; top: 5px; right: 5px; height: 30px; width: 30px; margin: 0; padding: 0; outline: none; border: none; -webkit-appearance: none;">
...
<table width=100%>
  ...
  <tr>
    ...
    <tr>
      ...
      <td><input name=fromdate size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)" value="$form->{fromdate}"> <b>|
      . $locale->text('To')
      . qq|</b> <input name=todate size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)" value="$form->{todate}"></td>
    </tr>
    $selectfrom
    <tr>
