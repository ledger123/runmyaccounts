...
|;
    ...
    $column_data{datepaid} = qq|<td align=center nowrap><input name="discount_datepaid" size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)" value=$form->{"discount_datepaid"}></td>|;
  ...
  <tr>
    <td>
	  ...
	  <td>
		...
		<td colspan=3><input name=transdatefrom size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)"> <b>|.$locale->text('To').qq|</b> <input name=transdateto size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)"></td>
	      </tr>
	      $selectfrom
	    </table>
	  </td>
...
sub transactions {
  ...
  @columns = $form->sort_columns(qw(transdate id invnumber ordnumber ponumber description name customernumber vendornumber address netamount tax amount paid paymentmethod due curr datepaid duedate memo notes intnotes till employee manager warehouse shippingpoint shipvia waybill dcn paymentdiff department));
...
|;
  ...
  foreach $ref (@{ $form->{transactions} }) {
    ...
    for (qw(department ordnumber ponumber notes intnotes warehouse shippingpoint shipvia waybill employee manager till source memo description projectnumber address dcn paymentmethod)) { $column_data{$_} = "<td>$ref->{$_}&nbsp;</td>" }
...
sub transactions_to_csv {
  ...
  foreach $ref (@{ $form->{transactions} }) {
    ...
    for (qw(department ordnumber ponumber notes intnotes warehouse shippingpoint shipvia waybill employee manager till source memo description projectnumber address dcn paymentmethod)) { $column_data{$_} = &escape_csv($ref->{$_} . " ") }
