...
qq|<option value="$myconfig{department}--$myconfig{department_id}">$myconfig{department}\n|;
		...
		while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
			$form->{"$selectname"} .=
qq|<option value="$ref->{description}--$ref->{id}">$ref->{description}\n|;
		}
	}
}
...
sub bld_warehouse {
	...
	my $where = "1 = 1";
	$query =
qq|SELECT id, description FROM warehouse WHERE $where ORDER BY description|;
	$dbh = $form->dbconnect( \%myconfig );
	$sth = $dbh->prepare($query);
...
sub print_select {
	...
	print
qq|<td><select name=$fldname>$form->{"select$fldname"}</select></td></tr>\n|;
}

###############################
sub print_radio {
	print qq|
	<input name=summary type=radio class=radio value=1 checked> |
	  . $locale->text('Summary') . qq|
	<input name=summary type=radio class=radio value=0> |
	  . $locale->text('Detail') . qq|<br>
   |;
}
...
sub bld_employee {
	...
	while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		$form->{"select$fldname"} .=
		  qq|<option value="$ref->{name}--$ref->{id}">$ref->{name}\n|;
	}
}
...
sub bld_chart {
	...
	while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		$form->{"$selectname"} .=
qq|<option value="$ref->{accno}--$ref->{id}">$ref->{accno}--$ref->{description}\n|;
	}
}
...
sub print_date {
	...
	print
	  qq|<tr><th align=right>$fldprompt</th><td><input type=text name=$fldname 
...
sub print_checkbox {
	...
	print
qq|<input name=$fldname class=checkbox type=checkbox value=Y $checked> $fldprompt\n|;
...
sub rpt_hdr {
	...
	if ($href) {
		if ($align) {
			$str =
qq|<th><a class=listheading align=$align href=$href&sort=$column_name>$column_heading</a></th>|;
		}
		else {
			$str =
qq|<th><a class=listheading href=$href&sort=$column_name>$column_heading</a></th>|;
		}
	}
	else {
		if ($align) {
			$str = qq|<th class=listheading align=$align>$column_heading</th>|;
		}
		else {
...
sub rpt_dec {
	...
	my $str =
	    qq|<td align=right>|
	  . $form->format_amount( \%myconfig, $column_data, $precision, $dash )
	  . qq|</td>|;
	$str;
}
...
sub rpt_print_date {
	...
	my $str =
qq|<td><input type=text name=$fldname size=11 title='$myconfig{dateformat}' value='$defaultvalue' class="date"></td>\n|;
	$str;
}
...
sub list_parts {
	...
	while ( my $ref = $sth->fetchrow_hashref(NAME_lc) ) {
		...
		print
qq|<td><input name=ndx class=radio type=radio value=$j $checked></td>\n|;
