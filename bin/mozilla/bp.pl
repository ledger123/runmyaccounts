...
# $locale->text('E-Mail')
  ...
  if ($form->{batch} ne 'queue') {
    ...
    @a = qw(invoice packing_list pick_list bin_list);
  ...
  if ($form->{type} eq 'timecard') {
    if (@{ $form->{all_project} }) {
      $projectnumber = qq|
	    ...
	    <td colspan=3><select name=projectnumber><option>\n|;

	    for (@{ $form->{all_project} }) { $projectnumber .= qq|<option value="$_->{projectnumber}--$_->{id}">$_->{projectnumber}\n| }
...
<table width=100%>
  ...
  <tr>
	...
	<tr>
	  ...
	  <td><input name=transdatefrom size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)">
...
# $locale->text('E-mailing')
  ...
  for my $i (1 .. $myform->{rowcount}) {
	...
	$inv = 'inv'
      ...
      $old_form->{$_} = $form->{$_} for qw|id intnotes|;
      $old_form->{batch} = 1;
      # unquote variables
	...
	$myform->info(qq|, $form->{"${inv}total"}, $form->{"$form->{vc}number"}, $form->{"$form->{vc}"} $form->{place}|);
      }
...
$column_header{vcnumber} = "<th><a class=listheading href=$href&sort=vcnumber>".$locale->text('Number')."</a></th>";
  ...
  $column_header{place} = "<th class=listheading>".$locale->text('City')."</th>";
...
|;
  ...
  foreach $ref (@{ $form->{SPOOL} }) {
    ...
    for (qw(description email cc place id invnumber ordnumber quonumber vcnumber)) { $column_data{$_} = qq|<td>$ref->{$_}</td>| }
