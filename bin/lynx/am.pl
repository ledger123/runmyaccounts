...
sub generate_yearend {
  ...
  if ($ok) {
    ...
    $form->error($locale->text('Could not close the period because there are no bookings.'));
  }
...
|;
	...
	$column_data{nextdate} = qq|<td nowrap><input name="nextdate_$k" size=11 value="$ref->{nextdate}" title="$myconfig{dateformat}" onChange="validateDate(this)"></td>|;
      }
...
<table width=100%>
  ...
  <tr>
	...
	<tr>
	  <td></td>
	  <td><input name=transdate value="$form->{transdate}" size=11 class=date title="$myconfig{dateformat}" onChange="validateDate(this)"></td>
