...
\begin{tabularx}{\textwidth}{@{}rlXlrlrrr@{}}
  \textbf{Pol} & \textbf{ÄÃ­slo} & \textbf{Popis} & & \textbf{MnoÅ¾.} &
...
\begin{tabularx}{\textwidth}{*{8}{|X}|} \hline
  \textbf{FaktÃºra} & \textbf{DÃ¡tum} & \textbf{ObjecnÃ¡vka} & \textbf{Pred.osoba}
  <%if shippingpoint%>
  & \textbf{Miesto urÄenia}
  <%end shippingpoint>
  <%if ponumber%>
  & \textbf{PO ÄÃ­slo}
  <%end ponumber%>
...
<%end taxincluded%>
<%foreach tax%>
 DIÄ. <%taxnumber%> & <%taxdescription%> on <%taxbase%> @ <%taxrate%> \% & <%tax%> \\
<%end tax%>
<%if not taxincluded%>
 & Celkom & <%invtotal%> \\
<%end taxincluded%>
