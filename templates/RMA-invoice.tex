...
\begin{document}
\AddToShipoutPicture{\BackgroundPic} % MUSS EINKOMMENTIERT WERDEN WENN
HINTERGRUND
\shorthandoff{"}
\pagestyle{empty}

% Setzt die generelle Position der Seite. Der bedruckbare bereich kann
horizontal / vertikal verschoben werden.
...
\textbf{Beschreibung} & \textbf{Menge} & \textbf{Einheit} &
\textbf{Preis <%currency%>} & \textbf{Betrag <%currency%>} \\
\hline
...
% Bei linker Positionierung des Adresskopfs / 2.25cm muss bei
Nicht-Pingen Templates auf den Seitenrand unter \usepackage[a4paper, ...
angepasst werden.
...
\underline{\tiny{<%company%>, \BeforeSubString{ --}{<%address%>},
...
\ifthenelse{\equal{<%contact%>}{}}{}{<%salutation%> <%firstname%>
<%lastname%>}
...
Bestellnummer: <%customerponumber%>}
...
<%discountrate%> \%} & <%unit%>
\ifthenelse{\equal{<%discountrate%>}{}}{}{\newline Rabatt}&
<%sellprice%> \ifthenelse{\equal{<%discountrate%>}{}}{}{\newline
<%netprice%>}  & <%linetotal%>\\
<%end number%>
\end{tabularx}
...
<%if total%>
\textbf{Rechnungsbetrag <%currency%>\ifthenelse{\equal{<%tax%>}{}}{
(ohne MWST)} { inkl. MWST}}& \textbf{<%invtotal%>}\\
<%end total%>
...
\parbox{\textwidth}{
\ifthenelse{\equal{<%terms%>}{0}}{Wir bedanken uns für die
\textbf{sofortige Überweisung.}}{Wir bedanken uns für Ihre
\textbf{Überweisung innerhalb von <%terms%> Tagen} bis zum <%duedate%>.}
}
...
%\AddToShipoutPicture{\BackgroundPicEZ} % MUSS EINKOMMENTIERT WERDEN
WENN HINTERGRUND
...
%DIESE BEIDEN KLAMMERN WERDEN GEBRAUCHT, WENN DIE EINFÜGUNG DES EZ
AUTOMATISCH ERFOLGT
}
}
}
