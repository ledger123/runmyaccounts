...
get '/' => sub ($c) {
    ...
    my $account_desc = $dbs->query( "SELECT description FROM chart WHERE accno = ?", $clearing_account )->list || 'Unknown Account';
...
post '/book_selected' => sub ($c) {
    ...
    if ($gl_account_id) {
        $gl_account_details = $dbs->query( "SELECT accno, description FROM chart WHERE id = ?", $gl_account_id )->hash || {};
    }

    $c->render(
        template              => 'book_confirmation',
...
post '/process_adjustment' => sub ($c) {
    ...
    eval {
        ...
        my $clearing_accno_id = $dbs->query( "SELECT id FROM chart WHERE accno = ?", $clearing_account )->list;

        my $transition_accno_id = $dbs->query( "SELECT id FROM chart WHERE accno = ?", $transition_account )->list;

        # Simple GL account change (no AR/AP transactions selected)
        if ( $gl_account_id && !$selected_ids ) {
            $dbs->query( "UPDATE acc_trans SET chart_id = ? WHERE chart_id = ? AND trans_id = ?", $gl_account_id, $clearing_accno_id, $trans_id );
        ...
        if ($selected_ids) {
            ...
            # Get GL transaction details
            my ( $gl_date, $curr, $fxrate ) = $dbs->query( "SELECT transdate, curr, COALESCE(exchangerate, 1) FROM gl WHERE id = ?", $trans_id )->list;

            $fxrate ||= 1;    # Default to 1 if null

            # Get the adjustment amount available from GL
            my $adjustment_available =
              $dbs->query( "SELECT 0 - amount FROM acc_trans WHERE chart_id = ? AND trans_id = ? AND NOT COALESCE(fx_transaction, false)", $clearing_accno_id, $trans_id )->list || 0;
            ...
            for my $row (@rows) {
                ...
                my $arap_accno_id = $dbs->query( "SELECT chart_id FROM acc_trans WHERE trans_id = ? AND chart_id IN (SELECT id FROM chart WHERE link = ?) LIMIT 1", $row->{id}, "${ARAP}" )->list;

                if ( $arap eq 'ap' ) {

                    # AP adjustments - exactly as in legacy code
                    $dbs->query( "INSERT INTO acc_trans(trans_id, chart_id, transdate, amount, id) VALUES (?, ?, ?, ?, ?)",
                        $row->{id}, $transition_accno_id, $payment_date, $amount_to_be_adjusted, $payment_id );

                    $dbs->query( "INSERT INTO acc_trans(trans_id, chart_id, fx_transaction, transdate, amount) VALUES (?, ?, ?, ?, ?)",
                        $row->{id}, $transition_accno_id, 't', $payment_date, $fx_amount_to_be_adjusted );

                    $dbs->query( "INSERT INTO acc_trans(trans_id, chart_id, transdate, amount) VALUES (?, ?, ?, ?)",
                        $row->{id}, $arap_accno_id, $payment_date, ( $row->{fxdue} * -1 ) + ( ( $row->{fxdue} * $fxrate - $row->{fxdue} ) * -1 ) );

                } else {

                    # AR adjustments - exactly as in legacy code
                    $dbs->query( "INSERT INTO acc_trans(trans_id, chart_id, transdate, amount, id) VALUES (?, ?, ?, ?, ?)",
                    ...
                    $dbs->query( "INSERT INTO acc_trans(trans_id, chart_id, transdate, amount) VALUES (?, ?, ?, ?)",
                        $row->{id}, $arap_accno_id, $payment_date, $row->{fxdue} + ( $row->{fxdue} * $fxrate - $row->{fxdue} ) );
                }

                # Insert payment record if payment_id was generated
                if ($payment_id) {
                    $dbs->query( "INSERT INTO payment (id, trans_id, exchangerate) VALUES (?, ?, ?)", $payment_id, $row->{id}, $fxrate );
                }
            ...
            if ( $adjustment_total != 0 ) {
                ...
                $dbs->query( "INSERT INTO acc_trans (trans_id, chart_id, amount, transdate) VALUES (?, ?, ?, ?)", $trans_id, $transition_accno_id, $adjustment_total, $gl_date );

                # Add FX transaction for transition account
                my $fx_adjustment = $adjustment_total * $fxrate - $adjustment_total;
                if ( $fx_adjustment != 0 ) {
                    $dbs->query( "INSERT INTO acc_trans (trans_id, chart_id, amount, transdate, fx_transaction) VALUES (?, ?, ?, ?, ?)",
                        $trans_id, $transition_accno_id, $fx_adjustment, $gl_date, 't' );
                }

                # Reduce the clearing account amount
                $dbs->query( "UPDATE acc_trans SET amount = amount - ? WHERE chart_id = ? AND trans_id = ? AND NOT COALESCE(fx_transaction, false)", $adjustment_total, $clearing_accno_id, $trans_id );

                # Check if clearing account amount is now zero and delete if so
                my $remaining_amount = $dbs->query( "SELECT amount FROM acc_trans WHERE chart_id = ? AND trans_id = ? AND NOT COALESCE(fx_transaction, false)", $clearing_accno_id, $trans_id )->list;

                if ( defined $remaining_amount && $remaining_amount == 0 ) {
                    $dbs->query( "DELETE FROM acc_trans WHERE chart_id = ? AND trans_id = ? AND NOT COALESCE(fx_transaction, false)", $clearing_accno_id, $trans_id );
                }
            }
