...
sub invoice_details {
  ...
  foreach $item (@sortlist) {
	...
	if ($ok) {
	  ...
	  for (qw(taxrates runningnumber number sku serialnumber ordernumber customerponumber bin qty ship unit deliverydate projectnumber sellprice listprice netprice discount discountrate discountrate_percent linetotal itemnotes package netweight grossweight volume countryorigin hscode barcode xml_linetotal)) { push(@{ $form->{$_} }, "") }
  ...
  $form->format_string(qw(text_amount text_decimal text_cd_invtotal text_cd_decimal text_out_decimal text_out_amount));
...
sub delete_invoice {
  ...
  if ($form->{id} and $defaults{extendedlog}) {
     $query = qq|INSERT INTO ap_log_deleted SELECT ap.* FROM ap WHERE id = $form->{id}|;
...
sub post_invoice {
  ...
  for $i (1 .. $form->{rowcount}) {
	...
	$diff = ($amount - $tax);
      ...
      $query = qq|UPDATE invoice SET
		  ...
		  discount = |.$form->dbclean($form->{"discount_$i"}).qq|,
		  allocated = $allocated,
         ...
         $query = qq|INSERT INTO inventory (
			...
			cost)
                VALUES ($form->{warehouse_id}, $form->{"id_$i"}, $form->{id},
                        ...
                        qq|, |.$form->dbclean($form->{employee_id}).qq|, |.$form->dbclean($form->{department_id}).qq|, | .
	  ...
	  while (my $ref = $sth->fetchrow_hashref(NAME_lc)) {
	    ...
	    if ($linetotal) {
	      ...
	      $query = qq|INSERT INTO acc_trans (trans_id, chart_id, amount, 
			  ...
			  $linetotal, |.$dbh->quote($form->{transdate}).qq|, $ref->{project_id},
			  $id)|;
	      $dbh->do($query) || $form->dberror($query);
	      ...
	      $query = qq|INSERT INTO acc_trans (trans_id, chart_id, amount, 
			  ...
			  $linetotal * -1, |.$dbh->quote($form->{transdate}).qq|,
			  $ref->{project_id}, $id)|;
	      $dbh->do($query) || $form->dberror($query);
	    }
  ...
  if ($form->{payables}) {
    $query = qq|INSERT INTO acc_trans (trans_id, chart_id, amount,
		       ...
		       WHERE accno = |.$dbh->quote($apaccno).qq|),
                $form->{payables}, |.$dbh->quote($form->{transdate}).qq|)|;
    $dbh->do($query) || $form->dberror($query);
  }
  ...
  for $i (1 .. $form->{paidaccounts}) {
	...
	$cleared = $form->dbquote($form->dbclean($form->{"cleared_$i"}), SQL_DATE);
      ...
      $query = qq|INSERT INTO acc_trans (trans_id, chart_id, amount, transdate,
                  ...
                  $form->{"paid_$i"}, |.$dbh->quote($form->{"datepaid_$i"}).qq|, |
...
sub retrieve_item {
  ...
  my $vwhere = qq|$where
                  AND pv.vendor_id = |.$form->dbclean($form->{vendor_id}).qq||;
  ...
  my $query = qq|SELECT p.id, p.partnumber, p.description,
		 ...
		 p.inventory_accno_id, p.income_accno_id, p.expense_accno_id,
		 p.partnumber AS sku, p.weight, p.project_id, project.projectnumber,
