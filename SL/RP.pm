    ...
    foreach $category (@categories) {
        ...
        foreach $key ( sort keys %{ $form->{$category} } ) {
            ...
            if ( $form->{$category}{$key}{charttype} eq "H" ) {
                if ( $account{$category}{subtotal} && $form->{l_subtotal} ) {
                    ...
                    push(
                        ...
                        $form->format_amount( $myconfig, $account{$category}{subthis} * $account{$category}{ml}, $form->{decimalplaces}, $dash )
                    );

                    if ($last_period) {
                        push(
                            @{ $form->{"$account{$category}{labels}_last_period"} },
                            $form->format_amount( $myconfig, $account{$category}{sublast} * $account{$category}{ml}, $form->{decimalplaces}, $dash )
                        );
                    }
            ...
            if ($last_period) {
                ...
                push(
                    @{ $form->{"$account{$category}{labels}_last_period"} },
                    $form->format_amount( $myconfig, $form->{$category}{$key}{last} * $account{$category}{ml}, $form->{decimalplaces}, $dash )
                );
            }
        }
...
sub aging {
    ...
    if ( $form->{payed} ) {
        $where = qq|
	a.approved = '1'
	AND ((a.amount = a.paid AND (SELECT MAX(acc.transdate) FROM acc_trans acc WHERE acc.trans_id = a.id) >= |
          . $dbh->quote( $form->{todate} )
          . qq|) OR a.amount <> a.paid OR a.datepaid > |
          . $dbh->quote( $form->{todate} )
          . qq| OR a.datepaid IS NULL)
	AND c.id = ?
	AND a.curr = ?|;
    }
    ...
    my %c = (
        c0  => { flds => '(a.amount - a.paid) AS c0, 0.00 AS c15, 0.00 AS c30, 0.00 AS c45, 0.00 AS c60, 0.00 AS c75, 0.00 AS c90' },
    ...
    if ( $form->{payed} ) {
        $openAmountQuery =
            qq| round(a.amount::numeric, 2) - round((coalesce((SELECT SUM(ac.amount) FROM acc_trans ac JOIN chart ch ON (ch.id = ac.chart_id) WHERE ac.trans_id = a.id and ac.transdate <= |
          . $dbh->quote( $form->{todate} )
          . qq| AND ac.approved = '1' AND (ch.link LIKE '%|
          . $upperCaseARAP
          . qq|_paid%' or ch.id in (select fldvalue::int from defaults where fldname in ('fxgain_accno_id','fxloss_accno_id')))), 0) * |
          . $openAmountMultiplicator
          . qq|)::numeric,2)|;
        %c = (
            c0  => { flds => $openAmountQuery . qq| AS c0, 0.00 AS c15, 0.00 AS c30, 0.00 AS c45, 0.00 AS c60, 0.00 AS c75, 0.00 AS c90| },
