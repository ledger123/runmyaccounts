...
sub purchase_invoice {
  ...
  for (@d) {
	...
	$form->{"${_}_$i"} = $a[$form->{$form->{type}}->{$_}{ndx}];
      ...
      if ($vendornumber ne $a[$form->{$form->{type}}->{vendornumber}{ndx}] || $invnumber ne $a[$form->{$form->{type}}->{invnumber}{ndx}]) {
...
sub import_purchase_invoice {
  ...
  my $language_code;
  $query = qq|SELECT c.vendornumber, c.language_code, a.city
              FROM vendor c
	      JOIN address a ON (a.trans_id = c.id)
	      WHERE c.id = $form->{vendor_id}|;
  ...
  # post invoice
  my $rc = IR->post_invoice($myconfig, $form, $dbh);
