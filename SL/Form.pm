...
sub save_recurring {
    ...
    if ( $self->{recurring} ) {
		...
		'$s{startdate}', '$enddate', | . $self->dbquote( $nextdate, SQL_DATE ) . qq|, $s{repeat}, '$s{unit}', $s{howmany}, '$s{payment}')|;
        $dbh->do($query) || $self->dberror($query);
...
sub audittrail {
    ...
    if ( $audittrail->{id} *= 1 ) {
        ...
        if ( $defaults{audittrail} ) {
		    ...
		    $audittrail->{id}, | . $dbh->quote( $audittrail->{tablename} ) . qq|, | . $dbh->quote( $audittrail->{reference} ) . qq|',
		    ...
		    | . $dbh->quote( $audittrail->{tablename} ) . qq|, | . $dbh->quote( $audittrail->{reference} ) . qq|,
		    | . $dbh->quote( $audittrail->{formname} ) . qq|, | . $dbh->quote( $audittrail->{action} ) . qq|,
